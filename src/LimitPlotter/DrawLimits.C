...
void CompareLimits(){
  ...
  files.push_back(filepath+"/AddSyst_DividedBy1p87/2017_MuMu_HNL_UL_Asym_limit.txt");
  files.push_back(filepath+"/AddSyst_intact/2017_MuMu_HNL_UL_Asym_limit.txt");
  files.push_back(filepath+"/AddSyst_intact/2017_EE_HNL_UL_Asym_limit.txt");
  files.push_back(filepath+"/AddSyst_intact/2017_EMu_HNL_UL_Asym_limit.txt");
  ...
  for(int i=0; i<files.size(); i++){
    ...
    while(getline(in,elline)){
      ...
      if(elline.find("7500") != string::npos){
        n_central--;
        continue; //FIXME JH : 75 now has a xsec issue...
      }
      ...
      double scale=0.01; //mixing squared is .1 now
